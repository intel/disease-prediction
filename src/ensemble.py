...
class Ensemble(object):
    ...
    def ensemble(self, config):
        ...
        if (len(nlp_corr_pred) != len(vision_corr_pred)) or ( all(nlp_corr_pred.id != vision_corr_pred.id)):
            nlp_corr_pred, vision_corr_pred = set_diff_ids(nlp_corr_pred, vision_corr_pred) 
...
def f1_score_per_class(yaml_dict):
    ...
    return f1_scores, df_cm_res.loc['accuracy'][0]
...
def corrected_prediction_prob(config, domain=None):
    ...
    f1_scores, acc = f1_score_per_class(yaml_dict_finetune)
            
    # corrected scores based on f1-scores
    corrected_scores = [
        list( f1_scores**(1/3) * i * acc**3 ) for i in yaml_dict_inference["predictions_probabilities"]
    ]
    ...
    df_corrected["raw_predictions_probabilities"] = yaml_dict_inference["predictions_probabilities"]
