...
def main(_):
  with tf.device('/gpu:0'):
      ...
      if tf.compat.v1.flags.FLAGS.do_train:
        tf.compat.v1.logging.info("***** Running training *****")
        tf.compat.v1.logging.info("  Batch size = %d", FLAGS.train_batch_size)
        train_input_fn = input_fn_builder(
            input_files=input_files,
            max_seq_length=tf.compat.v1.flags.FLAGS.max_seq_length,
            max_predictions_per_seq=tf.compat.v1.flags.FLAGS.max_predictions_per_seq,
            is_training=True)
        
        estimator.train(input_fn=train_input_fn, max_steps=tf.compat.v1.flags.FLAGS.num_train_steps)

      if tf.compat.v1.flags.FLAGS.do_eval:
        tf.compat.v1.logging.info("***** Running evaluation *****")
        tf.compat.v1.logging.info("  Batch size = %d", tf.compat.v1.flags.FLAGS.eval_batch_size)
        ...
        result = estimator.evaluate(
            input_fn=eval_input_fn, steps=tf.compat.v1.flags.FLAGS.max_eval_steps)

        output_eval_file = os.path.join(tf.compat.v1.flags.FLAGS.output_dir, "eval_results.txt")
